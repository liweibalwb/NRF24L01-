...
//	u16 tempcolor;
	...
	{	 
	    ...
	    {   												   
	        ...
	        for(t1=0;t1<8;t1++)
			{
				if(temp&0x80)TFT_DrawPoint(x,y,color);
	 			else 
				{
				//	tempcolor=POINT_COLOR;
				//	POINT_COLOR=BACK_COLOR;
					TFT_DrawPoint(x,y,BACK_COLOR);
...
//正点原子@HYW
//CHECK:09/10/30			   
void Show_Str(u16 x,u16 y,u16 color,u8*str,u8 size,u8 mode)
{												  	  
    u8 bHz=0;     //字符或者中文  	    				    				  	  
    while(*str!=0)//数据未结束
    { 
        if(!bHz)
        {
	        ...
	        {      
                if(x>(TFT_W-size/2))//换行
				{				   
					y+=size;
					x=0;	   
				}							    
		        if(y>(TFT_H-size))break;//越界返回      
		        if(*str==10)//换行符号"\n"
		        {         
		            y+=size;
					x=0;
		        ...
		        else TFT_ShowChar(x,y,color,*str,size,mode);//有效部分写入 
				str++; 
		        x+=size/2; //字符,为全字的一半 
	        }
        }else//中文 
        {     
	        ...
	        if(y>(TFT_H-size))break;//越界返回  						     
	        Show_Font(x,y,color,str,size,mode); //显示这个汉字,空心显示 
	        str+=2; 
	        x+=size;//下一个汉字偏移	    
        }						 
    }   
...
void Show_Str_Mid(u16 x,u16 y,u16 color ,u8*str,u8 size,u8 len)
{
	...
	strlenth*=size/2;
	if(strlenth>len)Show_Str(x,y,color,str,size,1);
	else
	{
		strlenth=(len-strlenth)/2;
	    Show_Str(strlenth+x,y,color,str,size,1);
	}
}   
