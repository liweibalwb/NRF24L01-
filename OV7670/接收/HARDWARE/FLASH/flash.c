...
void SPI_Flash_Init(void)
{
	GPIO_InitTypeDef  GPIO_InitStructure;					 //定义GPIO初始化结构体
 	...
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出,因为VS1053与W25X16共用SPI1，所以这里要把他的片选拉高
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度50MHZ
 	GPIO_Init(GPIOB, &GPIO_InitStructure);					
 	GPIO_SetBits(GPIOB,GPIO_Pin_5 | GPIO_Pin_6 |GPIO_Pin_12); //输出高

	SPI2_Init();		  									//初始化SPI
	SPI2_SetSpeed(SPI_BaudRatePrescaler_4);					//18MHZ
}  
